...
function [irf_Dimp,irf_CImean,CIup,CIlow,IRFse]=irf_VAR_MCMC_struct(betaNL,OmegaNL,Omegamat,betamat,s_desc)
...
% impulse response function. We use draws from the generated chain of
% parametervalues. 
CIiters=1000  ;   %do matter but not as much as anticipated. 
display ("irf_var_mcmc_struct")
...
shockvec=CholOmegaNL(:,s_desc.KIX)/CholOmegaNL(s_desc.KIX,s_desc.KIX); %ska det delas här?antagligen för att få 1 unit shock
...
for i1=1:CIiters
    ...
    paramdraw0=betamat(drawposition,:)'; %draw from Beta coefficients
    paramdraw=unvec(paramdraw0,Nvar,Nvar*VARlags);

    % draw the covariance matrix of residuals (contemporanous relationship)
    OmegaNLdraw0=Omegamat(drawposition,:)'; %draw ffrom Omegamat
    ...
    %standardiserar chocken. Men varierar ursprungligen. 
    ...
    irf_imp=irf_lin;
    %skapar irf för en iteration
    for i=2:irf_hor
        irf_lin=A_NL*irf_lin;
        irf_imp=[irf_imp irf_lin];
    end
    %skapar en stor lista med CIters iterations(irf)
    irf_Dimp0mat(i1,:,:)=irf_imp;

end

% compute confidence intervals and standard errors, inget konstigt här
CIup=[];
CIlow=[];
IRFse=[];
irf_CImean=[];
for i=1:Nvar
    ...
    IRFse  = [IRFse; std(squeeze(irf_Dimp0mat(:,i,:)),1,1)];            %standard deviation
    irf_CImean = [irf_CImean; median(squeeze(irf_Dimp0mat(:,i,:)),1)];  %point estimation
end
